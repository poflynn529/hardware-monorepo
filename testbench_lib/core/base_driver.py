...
class BaseDriver:
    clock: Clock
    port: Any
    pre_delay_range: range
    post_delay_range: range
    ...
    async def _send(self) -> None:
        while self._transaction_queue:
            for _ in range(randint(self.pre_delay_range.start, self.pre_delay_range.stop)):
                await RisingEdge(self.clock)

            self.expect_callback(self._transaction_queue[0])
            await self._drive_transaction(self._transaction_queue.popleft())

            for _ in range(randint(self.post_delay_range.start, self.post_delay_range.stop)):
                await RisingEdge(self.clock)
